...
path <- "/Users/roli/Documents/vegsoup-data/jettenbach-mühldorf dta"
...
#	summed (total) canopy value
obj$canopy <- apply(sites(obj)[ , match(c("s1cov", "s2cov", "t1cov", "t2cov", "t3cov"), names(obj)) ], 1,
	function (x) {
	round((1 - prod(1 - x / 100)) * 100, digits = 0)
	} )
	
obj$canopy.tl <- apply(sites(obj)[ , match(c("t1cov", "t2cov", "t3cov"), names(obj)) ], 1,
	function (x) {
	round((1 - prod(1 - x / 100)) * 100, digits = 0)
	} )

obj$canopy.sl <- apply(sites(obj)[ , match(c("s1cov", "s2cov"), names(obj)) ], 1,
	function (x) {
	round((1 - prod(1 - x / 100)) * 100, digits = 0)
	} )
