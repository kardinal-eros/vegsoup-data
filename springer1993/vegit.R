...
setwd("~/Documents/vegsoup-data/springer1993")

#	match taxa
file <- "Springer1993Tab14"
x <- paste0(file, ".csv")
csv2txt(x,  header.rows = 21, vertical = TRUE, width = 1, overwrite = FALSE)
...
#	NOT RUN:
if (FALSE) {
	xx <- extractTaxon(x, row = 26, col = 26)
	...
	r <- replaceTaxon(x, y, z, row = 26, col = 26,
		overwrite = TRUE, keywords = TRUE,
		file = paste0(file, "taxon2standard.txt"))
	
	###	footer species
	#	transform to matrix and ...
	xx <- castFooter(paste0(file,"FooterSpecies.txt"), species.first = FALSE, abundance.first = FALSE)
	#	 ... save for later use
