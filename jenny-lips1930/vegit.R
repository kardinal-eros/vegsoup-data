...
setwd("~/Documents/vegsoup-data/jenny-lips1930")

#	match taxa
file <- "Jenny-Lips1930Tab1"
x <- paste0(file, ".csv")
csv2txt(x, header.rows = 0, vertical = FALSE, width = 13, overwrite = FALSE)
...
#	NOT RUN:
if (FALSE) {
	xx <- extractTaxon(x, row = 3, col = 30)
	...
	r <- replaceTaxon(x, y, z, row = 3, col = 30,
		overwrite = FALSE, keywords = TRUE,
		file = paste0(file, "taxon2standard.txt"))
		
	###	footer species
	#	transform to matrix and ...
	xx <- castFooter(paste0(file, "FooterSpecies.txt"), abundance.first = FALSE)
	#	 ... save for later use
