...
path <- "~/Documents/vegsoup-data/reslberg dta"
bib <- read.bib(file.path(path, "references.bib"), encoding = "UTF-8"); key <- bib$key
key <- bib$key
key <- key[[1]]

file <- "~/Documents/vegsoup-data/reslberg dta/species wide.csv"
X <- stackSpecies(file = file, sep = ",")[, 1:4]

file <- "~/Documents/vegsoup-data/reslberg dta/sites wide.csv"
