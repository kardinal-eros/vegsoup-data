...
#	must manually edit results
	write.csv2(xy, file.path(path, "translate2.csv"), row.names = FALSE, quote = FALSE)
#	backup
	sh <- paste0("tar -zcvf",
		paste0(" \"", file.path(path, "translate2.tar.gz"), "\""),
		paste0(" --directory=\"", path, "\"", " translate2.csv"))
	system(sh)
#	read and join abbr
	x <- read.csv2(file.path(path, "translate2.csv"),
...
#	overwrite file
	write.csv2(x, file.path(path, "translate2.csv"),	row.names = FALSE, quote = FALSE)
}
