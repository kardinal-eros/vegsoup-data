...
file <- file.path(path, "Krisai1996Tab8taxon2standard.txt")
x <- read.verbatim(file, "Aufnahme Nr.", verbose = FALSE, layers = "@")
X0 <- species(x)

#	and footer taxa
file <- file.path(path, "Krisai1996Tab8FooterSpecies.csv")
X1 <- species(file, sep = ";")[, 1:4]

X <- bind(X0, X1)

#   sites data including coordinates
file <- file.path(path, "Krisai1996Tab8Locations.csv")
