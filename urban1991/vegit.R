...
setwd("~/Documents/vegsoup-data/urban1991")

#	match taxa
file <- "Urban1991Tab3"
x <- paste0(file, ".csv")
csv2txt(x,  header.rows = 11, vertical = FALSE, width = 4, overwrite = FALSE)
...
#	NOT RUN:
if (FALSE) {
	xx <- extractTaxon(x, row = 13, col = 38)
	...
	r <- replaceTaxon(x, y, z, row = 13, col = 38,
